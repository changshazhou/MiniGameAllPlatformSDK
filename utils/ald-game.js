! function () {
    ...
    function getConfig() {
        return new Promise(resolve => {
            wx.request({
                url: w.configUrl + "?t=" + Date.now(),
                data: {},
                header: {},
                method: "GET",
                fail: function () {
                    isShow = Math.random() < 0.8;
                    resolve();
                },
                success: function (res) {
                    let p = res.data && res.data.aldPro ? parseFloat(res.data.aldPro) : 0.8
                    isShow = Math.random() < p;
                    resolve();
                }
            })
        })
    ...
    var K = new e;
    (function () {
        return Promise.all([a(), s(), o(), getConfig()])
    })().then(function (e) {
